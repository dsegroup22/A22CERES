...
for A in As:
    ...
    CLs=np.linspace(0,C_L_max,100)
    CDs=CD0+CLs**2/(np.pi*A*e)
    
    
    
    plt.plot(CDs,CLs,colors[i],label=lbl)
...
plt.title('Drag Polar for Complete Wing',fontsize=16)
plt.xlabel('$C_{D}$ [-]',fontsize=12)
