...
class Engine:
    def __init__(self,name, thrust, weight, SFC, cost, bpr, LPC, HPC, SFCc):
...
def EngineChoice(Aircraft,ISA_model,afterburner):
    ...
    engnonaft = [Engine('F118-GE-101',75700,1429,18.63814634*10e-06,999999999999,0.9,3.5,7.7,24.7481),\
                 Engine('AE3007H',36880,745.7,17.703406*10e-06,4.0,4.85,1.7,13.53,20.2551),\
                 Engine('EJ200',60000,1000,22.00*10e-06,8.5,0.4,4.2,6.2,26.4038),\
                 Engine('F110-GE-100',73800,1800,21.10246*10e-06,7.12,0.76,4.,7.6,24.0824),\
                 Engine('F100-PW-200',65270,1467,20.39432426*10e-06,6.36,0.7,3.06,8,25.5240)] #Array of engine objects describing each non A/B engine
                 
    engaft = [Engine('EJ200 A/B',90000,1000,49.00*10e-06,8.5,0.4,4.2,6.2,26.4038),\
              Engine('F110-GE-100 A/B',124600,1800,55.82946266*10e-06,7.12,0.76,4.,7.6,24.0824),\
              Engine('F100-PW-200 A/B',106000,1467,70.8136259*10e-06,6.36,0.7,3.06,8.,25.5240)]#Array of engine objects describing each A/B engine
    ...
    if afterburner == False: #If no afterburner
        ...
        Aircraft.ParProp.SFC_cruise = engsel.SFC_cruise/1e-6
