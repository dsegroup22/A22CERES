...
def initialize_conditions(segment):
    ...
    air_speedI  = segment.ind_air_speed                      #Indicated
    ...
    air_speed =  air_speedI * np.sqrt(1.225/conditions.freestream.density)
...
def update_velocity_vector_from_wind_angle(segment):
    ...
    # unpack
    conditions = segment.state.conditions 
    v_ind      = segment.ind_air_speed 
    SUAVE.Methods.Missions.Segments.Common.Aerodynamics.update_atmosphere(segment)
    v_mag =  v_ind * np.sqrt(1.225/conditions.freestream.density)
