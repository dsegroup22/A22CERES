...
#    betaw = atan(Vw/VL)
#    lac = 6    #distance between the side geometric center and landing center of gravity
#    CLvw = CLv0 + CLvbeta*betaw+CLvdeltar*deltar
#    Lv = 0.5*rho*(VL**2+Vw**2)*Sv*CLv
#    Sside = 250   #side surface area!!!!!!!!
#    CDside = 0.65
#    Fw = 0.5*rho*Vw**2*Sside*CDside
#    cwdif = Lv*lvi-Fw*lac
#    Svwind = Fw*lac/lvi/(0.5*rho*(VL**2+Vw**2)*CLv)
#    print(Svwind)
...
#    deltar = radians(25)
#    CLv = CLv0 + CLvbeta*beta+CLvdeltar*deltar 
#    rho = 0.089  #takeoff air density
#    Vmc = Aircraft.ParAnFP.V_cruise   #1.13*vstall!!!!!!
##    Lv = 0.5*rho*Vmc**2*Sv*CLv
##    oeidif =Lv*lvi-0.5*Aircraft.ParAnFP.T_to*Aircraft.ParLayoutConfig.y_engine#Aircraft.ParLayoutConfig.y_loc_eng
#    Svi_check = 22000/Aircraft.ParStruc.N_engines*Aircraft.ParLayoutConfig.y_eng_out/(lvi*0.5*rho*Vmc**2*CLv)
#    print(Svi_check)
#    
...
#==============================================================================    
    ...
    mac_v = ((2./3.) * crv * (1. + trvi + trvi**2.)/(1. + trvi))
