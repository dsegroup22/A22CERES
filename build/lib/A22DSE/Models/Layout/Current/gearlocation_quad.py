...
#OUTPUTS:
    ...
    while(itcount<10):    
        ...
        gamma=np.arctan((max(x_cg)-x_nose)/y_nose)
        beta=np.arctan(c/np.sqrt((max(x_cg)-x_nose)**2+y_nose**2-c**2))
        alpha=beta+gamma-np.pi/2
        y_main_min_turnover=y_nose+(x_main-x_nose)*np.tan(alpha)
