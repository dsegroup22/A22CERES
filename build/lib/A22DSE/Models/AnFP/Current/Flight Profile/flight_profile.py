...
def flightprofile(Aircraft,ISA):
...
#Input: Wing Surface Area [m2], weight [kg], CD0[-], Aspect Ratio[-], Oswald efficiency factor[-], Thrust-to-Weight[-]
#Ouput: flight profile graphs
    anfp=Aircraft.ParAnFP
    struc=Aircraft.ParStruc
    pl=Aircraft.ParPayload
    ...
    rho = ISA.rho0  #[kg/m3] #density at sea level
    ...
    SFC = anfp.SFC #16*10e-6 #[kg/N/s] #specific fuel consumption <subject to change>
    #SFC = 0.667
    T = anfp.T_to #200000 #[N] #maximum thrust at take-off <subject to change>
    mf_to = SFC*T
    
    CD_to = anfp.CD_to #0.2 #[-] #drag coefficient during 1st phase of take-off <subject to change>
    CL_to = anfp.CL_to #1 #[-] #lift coefficient during 1st phase of take-off <subject to change>
    
    t = 0 #[s] #initial time
    dt = 1 #[s] #time increment
    mu = 0.03 #[-] #frictional viscosity during 1st phase take-off of main and nose landing gear <subject to change>
    Vr = anfp.Vr #50 #[m/s] #speed at which nose should be rotated up <subject to change>
    ...
    while alt<=alt_scr:
        ...
        altitude.append(alt)
        distance.append(s)
    print ('end of loop 3',t)
    
    #2gdprint(len(altitude))    
    #t = 0
    TtoW = anfp.TtoW #get.par('TtoW') #0.68 #from pere <subject to change>
    ...
    while alt <= 20000:
        if alt <= 11000:
            ...
            M = V_max_roc/sqrt(gaR*(temp+273.1))
            M_climb.append(M)
            #print(V_max_roc)
    ...
    #            alt = altitude[-1]
            ...
            t = t + dt
            time.append(t)
    print ('end of loop 4',t)
    
    
#    payload_in = par.get('m_payload') #[kg]
    payload = pl.m_payload #[kg]
    dispersalrate = pl.disperRatePerTime #[kg/s]
    timeinject = payload / dispersalrate
    mf_cr = 0.1 #specific fuel consumption at cruise
    ...
    rho_cruise = ISA.ISAFunc([anfp.h_cruise])[1]
    TtoW = 0.5
    ...
    plt.ylabel('Altitude [km]')
    #plt.figure(3)
    #plt.plot(np.array(altitude[705:])/1000,np.array(M_climb))
